...
python main_binary.py --batch_size ${batch_size} --lr ${lr} --num_epoch ${epoch} \
    ...
    --project "test${dataset}/10.13" \
    --world_size ${world_size} \
    --lr_sche ${lr_sche} \
    --title gigapath-${mil_method}-${world_size}xb${batch_size}-${loss}-multi${multi_label}-drop${patch_drop}-pad${patch_pad}-epoch${epoch}-lr${lr}-balanced \
    # --eval_only
