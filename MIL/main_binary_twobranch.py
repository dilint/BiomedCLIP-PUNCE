...
def prepare_metadata(args):
    """
    從 CSV 檔案中讀取並準備訓練和測試集的元數據，並計算類別分佈 pi。
    """
...
def run_training_process(rank, world_size, args,
    ...
    for epoch in range(args.num_epoch):
        ...
        # MODIFICATION: 將 pi 和 target_weight 傳遞給 training_loop
...
# MODIFICATION: 更新函數簽名以接受 target_weight 和 pi
def training_loop(args, model, loader, optimizer, device, amp_autocast, criterions, loss_weights, scheduler, epoch, rank, target_weight, pi):
    ...
    for i, ((bag1, bag2), label, file_path) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            # === MODIFICATION START: GHLoss 計算邏輯 ===
            # 一致性損失
...
def main():
    ...
    # MODIFICATION: 在主進程中準備資料元數據，並接收 pi
    ...
    # MODIFICATION: 組合傳遞給子進程的參數，加入 pi
    spawn_args = (
        world_size, args,
        train_wsi_names, train_wsi_labels, train_cluster_labels, train_data_in_shared_memory,
        test_wsi_names, test_wsi_labels, test_data_in_shared_memory,
