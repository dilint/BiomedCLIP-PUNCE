...
from dataloader import C16DatasetSharedMemory, ClassBalancedDataset, C16DatasetTwoView
...
from trainer.gh_trainer import gh_training_loop
...
def prepare_metadata(args):
    ...
    if args.gh > 0:
        ...
        unique_classes, counts = np.unique(train_wsi_labels_fine, return_counts=True)
        pi_counts = np.zeros(args.num_classes[1])
        pi_counts[unique_classes] = counts
        pi_proportions = pi_counts / pi_counts.sum()
...
def run_training_process(rank, world_size, args, 
    ...
    if args.gh > 0:
        print_and_log(f'进程 {rank}: 启用 GHLoss (args.gh > 0)，使用双分支 (TwoView) 数据集。', args.log_file, args.no_log)
        ...
        print_and_log(f'进程 {rank}: 未启用 GHLoss，使用单分支 (SharedMemory) 数据集。', args.log_file, args.no_log)
    ...
    test_loader = None
    if rank == 0:
        val_collate = default_collate 
        val_loader = DataLoader(val_set, batch_size=1, shuffle=False, num_workers=0, collate_fn=val_collate)
        test_loader = DataLoader(test_set, batch_size=1, shuffle=False, num_workers=0, collate_fn=val_collate)
    
    target_weight = None
    if args.gh > 0:
        print_and_log(f'进程 {rank}: 为 GHLoss 加载 target weights (dim={args.target_dim})', 
        ...
        try:
            ...
            print_and_log(f'进程 {rank}: 成功从 {target_weight_path} 加载 target weights', 
                          args.log_file, args.no_log)
                          
        except FileNotFoundError:
            print_and_log(f'错误: 在 {target_weight_path} 未找到 Target weight。GHLoss 将被禁用。',
                          args.log_file, args.no_log)
            args.gh = 0 
    ...
    if args.gh > 0:
        criterions['con'] = GHLoss(0.05, 300)
        loss_weights['con'] = args.consistency_weight
    ...
    for epoch in range(args.num_epoch):
        ...
        if args.mil_method == 'hmil':
            ...
            if args.gh > 0:
                train_loss, start_time, end_time = gh_training_loop(
                    args, model, train_loader, optimizer, device,
        ...
        total_loss = train_loss.get('total', 0.0) 
        ...
        if rank == 0:
            ...
            print_and_log(f'Epoch [{epoch+1}/{args.num_epoch}] 验证集 Fine AUC: {current_val_auc:.4f}', 
                          args.log_file, args.no_log)
...
def main():
    ...
    val_wsi_names, val_wsi_labels, \
    test_wsi_names, test_wsi_labels, pi = prepare_metadata(args)
    ...
    if args.rank == 0:
        print_and_log("正在加载验证/测试集到共享内存 (仅 rank 0)...", args.log_file, args.no_log)
        val_data_in_shared_memory = C16DatasetSharedMemory.preload_to_shared_memory(
            val_wsi_names,
            root_dir=args.dataset_root,
...
def parse_args():
    ...
    parser.add_argument('--num_classes', default=6, type=int, help='Number of classes. For HMIL, this will be overridden by a list [coarse, fine].')
    ...
    parser.add_argument('--kmeans-k', default=10, type=int)
    ...
    parser.add_argument('--gh', default=0, type=int, help='是否启用 GHLoss (0=否, 1=是)')
    parser.add_argument('--consistency_weight', default=0.05, type=float, help='GHLoss 的损失权重')
    parser.add_argument('--target_dim', type=int, default=6, help='GHLoss 投影的目标维度')
    ...
    if args.gh > 0 and not args.train_cluster_path:
        raise ValueError("GHLoss (args.gh > 0) 需要 --train_cluster_path 才能使用 C16DatasetTwoView。")
    if args.gh > 0 and args.mil_method == 'hmil':
        raise ValueError("GHLoss (args.gh > 0) 当前不支持 hmil_method。")
