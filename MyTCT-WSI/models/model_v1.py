...
class Model_V1(nn.Module):
    def __init__(
        ...
        self.vit0 = VisionTransformer_WSI(
            in_dims=self.in_dims,
            num_heads=self.num_heads,
            num_classes=1,
        ...
        self.vit15 = VisionTransformer_WSI(
            in_dims=self.in_dims,
            num_heads=self.num_heads,
            num_classes=5,
            embed_dim=self.in_dims,
        ...
        self.vit68 = VisionTransformer_WSI(
            in_dims=self.in_dims,
            num_heads=self.num_heads,
            num_classes=3,
            embed_dim=self.in_dims,
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor = None):
        ...
        #print(x.shape)
        logits0 = self.vit0(x, mask) # shape [B, 1]
        logits15 = self.vit15(x, mask) # shape [B, 5]
        logits69 = self.vit68(x, mask) # shape [B, 4]

        logits = torch.cat((logits0, logits15, logits69), dim=-1) # shape [B, 10]
...
def build_model(args):
    ...
    criterion = AsymmetricCriterion(num_classes)
    # criterion.to(device)
