...
def run_script(mil):
    if mil == 'ab':
        ...
        os.system(f'CUDA_VISIBLE_DEVICES={CPU_ID1}, python3 mil.py --project={PROJECT_NAME} --dataset_root={DATASET_PATH} --label_path={LABEL_PATH} --model_path={OUTPUT_PATH} --datasets={DATASET} --input_dim={NUM_DIM} --cv_fold=1 --title={TITLE_NAME} --model=clam_sb --train_val --seed={SEED} --wandb')
...
os.system(f'CUDA_VISIBLE_DEVICES={CPU_ID1}, python3 eval.py --label_path={LABEL_PATH} --dataset_root={DATASET_PATH} --ckp_path={CHECKPOINT_PATH} --datasets=tct --input_dim={NUM_DIM} --model=clam_sb --seed={SEED}')
