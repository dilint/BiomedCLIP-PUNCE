...
class Transmil(nn.Module):
    ...
    def __init__(self,input_dim=512,pos_pos=0,pos='ppeg',peg_k=7,head=8):
        ...
        self.layer1 = TransLayer(dim=input_dim,head=head)
        self.layer2 = TransLayer(dim=input_dim,head=head)

        if pos == 'ppeg':
            self.pos_embedding = PPEG(dim=input_dim,k=peg_k)
        elif pos == 'sincos':
            self.pos_embedding = SINCOS(embed_dim=input_dim)
        elif pos == 'peg':
